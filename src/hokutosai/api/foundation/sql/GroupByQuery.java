...
public final class GroupByQuery extends SequentialQuery {
	...
	public GroupByQuery(String key) throws HokutosaiApiException
	{
	...
	public GroupByQuery(String key, SQLSortOrder order) throws HokutosaiApiException
	{
		if (key == null || key.equals("")) {
			throw new HokutosaiApiException(50000, "ソートの基準となるキーがnullまたは空です。");
		}
	...
	{
		if (sql.length() == 0) {
			throw new HokutosaiApiException(50000, "GROUP BY句はシーケンシャルクエリの先頭として使用することはできません。");
		}
